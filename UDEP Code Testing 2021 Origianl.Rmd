...
data <- data[-(1:2),]
# needed just for the time being while we have fake data - delete later 
questions <- data[1,]
```
...
plot1 <- data %>%
  filter(Q70 != "") %>%
  ggplot(aes(y = factor(Q70,levels=c("More food to eat than most households", "About the same amount of food to eat as most households", "Less food to eat than most households"),ordered=TRUE), fill = Q70)) +
  geom_bar(show.legend = FALSE) +
...
plot2 <- data %>%
  filter(!Q72 == "") %>%
    ggplot(aes(y = factor(Q72,levels=c("More healthy than other children", "About the same level of health as other children", "Less healthy than other children"),ordered=TRUE), fill = Q72)) +
  geom_bar(show.legend = FALSE) +
...
plot1 <- data %>%
  filter(Q130 != "") %>%
  ggplot(aes(y = factor(Q130,levels=c("More formal education than others", "About the same amount of formal education as others", "Less formal education than others"),ordered=TRUE), fill = Q130)) +
  geom_bar(show.legend = FALSE) +
  scale_fill_manual(values = c("#56B4E9", "#56B4E9","#56B4E9", "#56B4E9")) +
...
plot_edu124 <- data %>%
  filter( Q124 !="") %>%
  ggplot(aes(y = factor(Q124,levels=c("More technology skills than other households", "About the same amount of technology skills as other households", "Fewer technology skills than other households"),ordered=TRUE), fill = Q124)) + 
  geom_bar(show.legend = FALSE) + 
  scale_fill_manual(values = c("#56B4E9","#56B4E9","#56B4E9")) + 
