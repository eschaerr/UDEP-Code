...
```{r Demographics Graphs, fig.height=3,fig.width=8, warnings = FALSE}
...
disability_plot
grid.arrange(religion_plot1, religion_plot2, nrow = 2)
...
#RECODE Q34
data$Q34<-recode(data$Q34, 'Worse than most households (poorer)'=1, 'About the same as most households'=2, 'Better than most households (richer)'=3)
data$Q34b <- as.numeric(data$Q34)
#RECODE Q44
data$Q44<-recode(data$Q44, 'Worse than other households'=1, 'About the same as other households'=2, 'Better than other households'=3)
data$Q44b <- as.numeric(data$Q44)
...
plot1 <- data %>%
  filter(Q130 != "") %>%
  # can take this out when we remove ladder question data from the data
  filter(Q130 %in% c("More formal education than others", "About the same amount of formal education as others","Less formal education than others")) %>%
...
m_avg_unpaid <- t.test(data$Q102~data$Q9,data)$estimate[1] 
f_avg_unpaid <- t.test(data$Q102~data$Q9,data)$estimate[2] 
significant <- t.test(data$Q102~data$Q9,data)$p.value<.05
...
data$Q48b <- ifelse(data$Q48 == "My home is too small for my family's needs.",1,ifelse(data$Q48 == "My home is big enough for my family's needs.",2,ifelse(data$Q48 == "My home is too big for my family's needs.",3,ifelse(data$Q48 == "I don't have a place to call home.",4,NA)))) # Ordinal meaure for home size
```
