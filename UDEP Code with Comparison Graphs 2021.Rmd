---
title: "UDEP Code with Comparison Graphs 2021"
...
gender_plot <- data %>%
  ...
  scale_fill_manual(values=c("#003399", "sandybrown")) +
...
age_plot <- data %>% 
  ...
  geom_histogram(show.legend=FALSE,fill="#99CCCC")+
...
disability_plot <- data %>%
  ...
  geom_bar(show.legend = FALSE, fill = "#99CCCC") +
...
religion_plot1 <- data %>%
  ...
  geom_bar(show.legend = FALSE) +
  scale_fill_manual(values=c("#99CCCC", "#99CCCC")) +
...
religion_plot2 <- data %>%
  filter(!Q19b == "") %>%
  ggplot(aes(x = fct_infreq(Q19b), fill = Q19b)) +
  geom_bar(show.legend = FALSE, fill = "#99CCCC") +
...
Poverty40.genderavg <- aggregate(Q40b ~ Q9b, data=data, mean)
...
Poverty42.genderavg <- aggregate(Q42b ~ Q9b, data=data, mean)
# t test for sex disaggreagated values
...
plot1 <- data %>%
  filter(!is.na(Q9b)) %>%
  filter(!Q9b == "") %>%
  ggplot(aes(Q9b, Q28pd, fill = Q9b)) + geom_boxplot()+ 
...
plot1 <- ggplot(data, aes(Q6b, Q28pd,fill=Q6b)) + geom_boxplot()+ 
  ggtitle("Income Trends by Age") + 
  xlab("Age") + ylab("Income ($ per day)") + 
  scale_x_discrete(labels= c("0-40","40+")) +
  scale_fill_manual(values=c("#66B3B3","#BBDDDD"),
                         name="Gender",
                         breaks=c(0, 1),
                         labels=c("0-40", "40+"))
...
When asked to compare their household's household income and overall financial situation to other households in the community `r round(mean(data$Q34 == "Worse than most households (poorer)", na.rm = TRUE)*100)`% said they had a worse financial situation than most households, `r round(mean(data$Q34 == "About the same as most households", na.rm = TRUE)*100)`% said they had about the same financial situation as most households, and `r round(mean(data$Q34 == "Better than most households (richer)", na.rm = TRUE)*100)`% said they had a better financial situation than most households.

When asked to compare their household's access to basic services (critical services including electricity and energy, water and sanitation, refuse and waste removal) to other households in the community `r round(mean(data$Q38 == "Less basic services than most households", na.rm = TRUE)*100)`% said they had fewer basic services than most households, `r round(mean(data$Q38 == "About the same amount of basic services as most households", na.rm = TRUE)*100)`% said they had about the same amount of basic services as most households, and `r round(mean(data$Q38 == "More basic services than most households", na.rm = TRUE)*100)`% said they had more basic services than most households.

When asked to compare their ability to buy land to others in the community, `r round(mean(data$Q44 == "Worse than other households", na.rm = TRUE)*100)`% said they had a worse ability to buy land than other households, `r round(mean(data$Q44 == "About the same as other households", na.rm = TRUE)*100)`% said they had about the same ability to buy land as other households, and `r round(mean(data$Q44 == "Better than other households", na.rm = TRUE)*100)`% said they had a greater ability to buy land than other households.

```{r message=FALSE, warning=FALSE, fig.height=3,fig.width=8}
########################################################
########################################################

levels <- c('Worse than most households (poorer)','About the same as most households','Better than most households (richer)' )

plot1 <- data %>%
  filter(Q34 %in% levels) %>%
  ggplot(aes(x = factor(Q34,levels=levels,ordered=TRUE))) +  geom_bar(show.legend = FALSE, fill ="#99CCCC" ) +
...
levels2 <- c("Fewer basic services than most households","About the same amount of basic services as most households","More basic services than most households")
plot2 <- data %>%
  filter(Q38 %in% levels2) %>%
  ggplot(aes(x = factor(Q38,levels=levels2,ordered=TRUE))) +
  geom_bar(show.legend = FALSE, fill = "#99CCCC" ) +
#  scale_fill_manual(values = c("#003399", "#003399","#003399","#003399")) +
  ...
  labs(
    title = expression(atop("Comparison on", "Access to Basic Services")),
    x = "",
    y = "Count" 
  ) 
########################################################
...
########################################################
########################################################
levels3 <- c("Worse than other households","About the same as other households","Better than other households")

plot3 <- data %>%
  filter(Q44 %in% levels3) %>%
  ggplot(aes(x = factor(Q44,levels=levels3,ordered=TRUE))) +  geom_bar(show.legend = FALSE) +
  geom_bar(show.legend = FALSE, fill = "#99CCCC") +
...
When asked to compare the health of the children in their household to that of other children, `r round(mean(data$Q72 == "Less healthy than other children", na.rm = TRUE)*100)`% of participants said that their children were less healthy than children in other households, `r round(mean(data$Q72 == "About the same level of health as other children", na.rm = TRUE)*100)`% said that their children were about as healthy as children in other households, and `r round(mean(data$Q72 == "More healthy than other children", na.rm = TRUE)*100)`% said that their children were more healthy than children in other households.
...
#SKILLED STAFF
data$doctor <- grepl("Doctor",data$Q91)
data$midwife <- grepl("Trained midwife", data$Q91)
data <- data %>%
  mutate(Q91b = ifelse(doctor + midwife>=1,1,0))
...
data$HIV <- grepl("HIV/AIDS",data$Q65)
...
Healthy65.ovrlavg <- mean(data$Q170b, na.rm = TRUE) * 100
...
Healthy65hepb.ovrlavg <- mean(data$hepatitisb, na.rm = TRUE) * 1000
...
Healthy177foodborne.ovrlavg <- mean(data$Foodbornes_count, na.rm = TRUE) * 1000
...
data$Leprosy_count <- grepl("Leprosy",data$Q177)
...
data$Onchocerciasis_count <- grepl("Onchocerciasis",data$Q177)
...
data$yaws_count <- grepl("Yaws",data$Q177)
...
In the sample, and `r round(HealthyF170.ovrlavg,2)`% of participants had someone in their household been affected with a disease in the last 12 months.The following are the proportions for specific diseases:
...
Regarding health care access, `r round(Healthy69.ovrlavg,2)`% of the partcipants know of access to substance abuse treatment, `r round(Healthy83.ovrlavg,2)`% have access to health coverage, and `r round(Healthy85.ovrlavg,2)`% have current health insurance.
...
When asked to compare the health of the women in their household to that of other women in their community, `r round(mean(data$Q87 == "Less healthy than other women", na.rm = TRUE)*100)`% said that women in their household were less healthy than other women, `r round(mean(data$Q87 == "About the same level of health as other women", na.rm = TRUE)*100)`% said that women in their household were about as healthy as other women, and `r round(mean(data$Q87 == "More healthy than other women", na.rm = TRUE)*100)`% said that women in their household were more healthy than other women.
...
When asked to compare the newborns in their household to newborns in other households in the community `r round(mean(data$Q63 == "Less healthy than other households", na.rm = TRUE)*100)`% said that newborns in their household were less healthy than other newborns, `r round(mean(data$Q63 == "About the same level of health as other households", na.rm = TRUE)*100)`% said that newborns in their household were about the same level of health as other newborns, and `r round(mean(data$Q63 == "More healthy than other households", na.rm = TRUE)*100)`% said that newborns in their household were more healthy than other newborns.
...
levels2 <- c("Less support needed for a healthy birth than most households","About the same support needed for a healthy birth as most households","Better support needed for a healthy birth than most households")

plot2 <- data %>%
  filter(Q93 %in% levels2) %>%
  ggplot(aes(x = factor(Q93,levels=levels2,ordered=TRUE)))+
  geom_bar(show.legend = FALSE, fill = "#99CCCC" ) +
...
levels3 <- c("Less healthy than other households","About the same level of health as other households","More healthy than other households")


plot3 <- data %>%
  filter(Q63 %in% levels3) %>%
  ggplot(aes(x = factor(Q63,levels=levels3,ordered=TRUE)))+
  geom_bar(show.legend = FALSE, fill = "#99CCCC") +
...
plot_edu128 <- data %>% 
  ...
  geom_bar(position = "dodge",fill="#99CCCC") +  
  theme(legend.position = c(0.8, 0.8)) +
  coord_flip() +
  scale_x_discrete(drop=FALSE) +
...
plot_edu112 <- data %>% 
  ...
  facet_grid(rows=vars(fct_recode(factor(Q6b),
                                  "40 years old and younger"="1",
                                  "Above 40 years old"="0")))
...
data$partnered <- ifelse(data$Q180=="Yes",1,ifelse(data$Q180=="No",0,NA))
...
#  geom_histogram(binwidth = .5,show.legend=FALSE,fill="sandybrown")+
...
plot_gndr_unpaid1 <- age_and_sex %>% 
  ...
  geom_col(position="dodge")+
  scale_fill_manual(values=c("#003399","sandybrown", "#003399", "sandybrown", "#003399", "sandybrown"), name = "Gender") +
...
levels2 <- c("Less control over these decisions than most women","About the same amount of control over these decisions as most women","More control over these decisions than most women")

plot_gndr92 <- data %>% 
  filter(Q9=="Woman") %>% 
  filter(Q92 %in% levels2) %>%
  ggplot(aes(x=factor(Q92,levels = levels2, ordered = TRUE))) + 
  geom_bar(fill = "#99CCCC")  + 
  scale_x_discrete(labels = c("Less control over these decisions than most women" = "Less", "About the same amount of control over these decisions as most women" = "Same", "More control over these decisions than most women" = "More")) +
  labs(x="",
       y="Number of women",
...
levels3 <- c("Less property/land than most women in other households","About the same amount of property/land as most women in other households","More property/land than most women in other households")


plot_gndr110 <- data %>%
  filter(Q110!="") %>% 
  filter(Q110 %in% levels3) %>%
  ggplot(aes(x = factor(Q110,levels=levels3,ordered=TRUE)))+
  geom_bar(fill="#99CCCC")+
...
In the last 12 months, `r pct_married_female_15_above_assaulted`% of married female respondents 15 years and older reported that a household member had been a victim of physical, sexual, or psychological violence.

When respondents were asked to compare their physical, sexual, or psychological safety to those in other households, `r round(mean(data$Q88 == "Less safe from physical, sexual, or psychological violence than other households", na.rm = TRUE)*100)`% said they felt less safe from physical, sexual, or psychological violence than others in the community, `r round(mean(data$Q88 == "About the same level of safety from physical, sexual, or psychological violence as other households", na.rm = TRUE)*100)`% said they felt about as safe as others in the community, and `r round(mean(data$Q93 == "More safe safe from physical, sexual, or psychological violence than other households", na.rm = TRUE)*100)`% said they felt more safe from physical, sexual, or psychological vioence than others in the community.
...
water_source <- data %>%
  ...
  mutate(b = ifelse(Q192.1 %like% "%Public tap or standpipe%",1,0)) %>%
  mutate(c = ifelse(Q192.1 %like% "%Tubewell or borehole%",1,0)) %>%
  mutate(d = ifelse(Q192.1 %like% "%Protected dug well%",1,0)) %>%
  mutate(e = ifelse(Q192.1 %like% "%Protected spring%",1,0)) %>%
  mutate(f = ifelse(Q192.1 %like% "%Unprotected spring%",1,0)) %>%
  mutate(g = ifelse(Q192.1 %like% "%Rainwater collection%",1,0)) %>%
  mutate(h = ifelse(Q192.1 %like% "%Bottled water%",1,0)) %>%
  mutate(i = ifelse(Q192.1 %like% "%Cart with small tank%",1,0)) %>%
  mutate(j = ifelse(Q192.1 %like% "%Tanker-truck%",1,0)) %>%
  mutate(k = ifelse(Q192.1 %like% "%Surface water%",1,0)) %>%
  mutate(l = ifelse(Q192.1 %like% "%Other%",1,0)) %>%
  select(a:l)

water_source2 <- water_source %>%
  mutate(pipe = ifelse(a ==1,1,0),
         public_tap = ifelse(b ==1,1,0),
         well = ifelse(c==1 | d ==1,1,0),
         spring = ifelse(e==1 |f==1,1,0),
         rain_surface = ifelse(g ==1|k==1,1,0),
         vehicle = ifelse(i==1|j==1,1,0),
         other = l) %>%
  select(pipe:other)
...
#   summarise(mean(a), mean(b), mean(c), mean(d), mean(e), mean(f), 
#             mean(g),mean(h), mean(i), mean(j), mean(k), mean(l))
# provides totals
# total_water_counts <- water_source %>%
#   summarize(a = sum(a),b = sum(b), c = sum(c),d = sum(d),e = sum(e),f = sum(f),g = sum(g), h = sum(h),i = sum(i),j = sum(j),k = sum(k),l = sum(l))
# totals with groupings
total_water_counts <- water_source2 %>%
  summarize(pipe = sum(pipe), public_tap = sum(public_tap), well = sum(well),spring = sum(spring), rain_surface= sum(rain_surface),vehicle = sum(vehicle),other = sum(other))
...
When asked about how frequently a household member had to drink water they felt was unsafe `r round(mean(data$Q191 == "Never (0)", na.rm = TRUE)*100)`% said never, `r round(mean(data$Q191 == "Rarely (1-2 times in the prior 4 weeks)", na.rm = TRUE)*100)`% said rarely (1-2 times in the prior 4 weeks), `r round(mean(data$Q191 == "Sometimes (3-10 times in the prior 4 weeks)", na.rm = TRUE)*100)`% said sometimes (3-10 times in the prior 4 weeks), `r round(mean(data$Q191 == "Often (11-20 times in the prior 4 weeks)", na.rm = TRUE)*100)`% said often (11-20 times in the prior 4 weeks), and `r round(mean(data$Q191 == "Always (above 20 times in prior 4 weeks)", na.rm = TRUE)*100)`% said always (above 20 times in prior 4 weeks).
...
-    `r round(mean(water_source2$pipe == 1, na.rm = TRUE)*100)`% of participants said they had access to piped water into their dwelling, yard or plot.

-    `r round(mean(water_source2$public_tap == 1, na.rm = TRUE)*100)`% said they had access to public taps or standpipes.

-    `r round(mean(water_source2$well == 1, na.rm = TRUE)*100)`% said they had access to boreholes, tubewells, or a protected dug well.

-    `r round(mean(water_source2$spring == 1, na.rm = TRUE)*100)`% said they had access to a protected or unprotected spring.

-    `r round(mean(water_source2$rain_surface == 1, na.rm = TRUE)*100)`% said they had access to rainwater collection or surface water.

-   `r round(mean(water_source2$vehicle == 1, na.rm = TRUE)*100)`% said they had access to a cart with tank or tanker-truck.

-   `r round(mean(water_source2$other == 1, na.rm = TRUE)*100)`% said they had access to another water source.
...
plot1 <- data %>%
  ...
  geom_bar(show.legend = FALSE, fill = "#99CCCC") +
  scale_y_discrete(labels= c("1" = "Never (0)", "2" = expression(atop("Rarely (1-2 times in", "the prior 4 weeks)")), "3" = expression(atop("Sometimes (3-10 times in", "the prior 4 weeks)")), "4" = "Often (11-20 times in the prior 4 weeks)", "5" = expression(atop("Always (above 20 times", "in prior 4 weeks)")))) +
  labs(title = expression(atop("Unsafe Water Consumption", "Per Week")),
       x = "Numer of Times",
       y = "")

# plot2 <- total_water_counts %>%
#   pivot_longer(cols = a:l, names_to = "type", values_to = "count") %>%
#   ggplot(aes(y = type, x = count)) +
#   geom_col(show.legend = FALSE, fill = "#99CCCC") +
#   scale_y_discrete(labels= c("a" = expression(atop("Piped water into dwelling,","yard or plot")), 
#                              "b" = "Public taps or standpipes", 
...
#        x = "Count"
#   )
plot2_2 <- total_water_counts %>%
  pivot_longer(cols = pipe:other, names_to = "type", values_to = "count") %>%
  ggplot(aes(y = type, x = count)) +
  geom_col(show.legend = FALSE, fill = "#99CCCC") +
  scale_y_discrete(labels= c("pipe" = expression(atop("Piped water into dwelling,","yard or plot")), 
                             "public_tap" = "Public taps or standpipes", 
                             "well" = "Well",
                             "spring" = "Spring",
                             "rain_surface" = "Rain or Surface Water",
                             "vehicle" = "Cart or Tanker-truck",
                             "other" = "Other")) +
  labs(title = "Water Source Access",
       x = "Count"
  )
...
#  scale_fill_manual(values=c("#003399", "#003399","#003399", "#003399","#003399","#003399","#003399","#003399","#003399","#003399")) +
#  coord_flip()
...
ggplot(aes(x = as.factor(Q48.1b), fill = as.factor(Q48.1b))) +
  ...
  scale_fill_manual(values = c("#99CCCC", "#99CCCC","#99CCCC", "#99CCCC","#99CCCC")) +
...
On average families use electricity `r round(mean(data$Q44.1, na.rm = TRUE))` hours a day. Households also on average lose electricty `r round(mean(data$Q45, na.rm = TRUE))` times a week. The average length of a power outage is `r round(mean(data$Q46.1, na.rm = TRUE))` hours.
...
data$Q139c <- runif(nrow(data), min = 0, max = 25) # (delete when real data)
#data$Q139c <- (data$Q139b/data$Q143b) # hourly income # uncomment with real data
...
# for testing purposes
t.age.1 <- t.test(Q139c ~ Q6b, data = ttests.1) 
t.age.0 <- t.test(Q139c ~ Q6b, data = ttests.0) 
avg.8.2 <- data %>% 
  group_by(Q9b,Q13b) %>% 
  summarise(Q139c = mean(Q139c, na.rm = TRUE)) # avg hourly income by gender and if disabled
# for testing purposes 
t.dis.1 <- t.test(Q139c ~ Q13b, data = ttests.1) # female disabled t test 
t.dis.0 <- t.test(Q139c ~ Q13b, data = ttests.0) # male disabled t test 
prop.bank <- data %>%
  filter(Q6 >= "15")
...
plot2 <- data %>%
  ...
  scale_fill_manual(values=c("#99CCCC", "#99CCCC","#99CCCC","#99CCCC")) +
  xlab("") + ylab("Count") +
  coord_flip()
...
# 10. Plots for each design type

```{r}
# SET UP
#######################################################
######################################################
# subset data depending on the answers through Q167-Q169
## in the real setting, the organization will have the same answer for Q166, so this should work
prepost <- subset(data, Q166=="The organization/intervention has pre-post data")
comtre <- subset(data, Q166=="The organization/intervention has comparison group data")
contre <- subset(data, Q166=="The organization/intervention has control group data")
timeseries <- subset(data, Q166=="The organization/intervention has time series data")

# subset data depending on the answers through Q167-Q169
data1 <- subset(prepost, Q167 == "Pre")
data2 <- subset(prepost, Q167== "Post")
data1 <- subset(comtre, Q168=="Comparison Group")
data2 <- subset(comtre,Q168=="Treatment Group")
data1 <- subset(contre, Q169=="Control Group")
data2 <- subset(contre, Q169=="Treatment Group")

# create a vector that indicates whether it is data1 or data2 
data1$designtype <- rep(c("data1"), nrow(data1))
data2$designtype <- rep(c("data2"), nrow(data2))

# integrate data1 and data2
joineddata <- rbind(data1, data2)

#####################################################
#####################################################
```

# Data Comparsion Analysis

```{r}
if(joineddata$Q166=="The organization/intervention has pre-post data"){
  group1 <- "Pre"
  group2 <- "Post"
}else if (joineddata$Q166=="The organization/intervention has comparison group data"){
  group1 <- "Comparison"
  group2 <- "Treatment"
}else if(joineddata$Q166=="The organization/intervention has control group data"){
  group1 <- "Control"
  group2 <- "Treatment"
}
```
...
## Poverty
```{r}
# scatter plot (Income Per Day)
maledata <- subset(joineddata, Q9b == 0)
femaledata <- subset(joineddata, Q9b == 1)

plot1 <- ggplot(maledata,aes(x=designtype, y=Q28pd)) + 
    geom_point()+
    geom_jitter(width = 0.25)+
    xlab("Group Type") + ylab("Male Income Per Day") + 
    scale_x_discrete(labels= c("Data 1","Data 2"))

plot2 <- ggplot(femaledata,aes(x=designtype, y=Q28pd)) + 
    geom_point()+
    geom_jitter(width = 0.25)+
    xlab("Group Type") + ylab("Female Income Per Day") + 
    scale_x_discrete(labels= c("Data 1","Data 2"))
...
```{r}
# my function for printing out the text that I need
text <- function(test, description) {
  ifelse(test$p.value > 0.05,"However, the difference between the groups is not significant.",ifelse(test$estimate[1] > test$estimate[2], paste("A difference of means test indicates that significantly more respondents in the ", group1,"group",description,"."),paste("A difference of means test indicates that significantly more respondents in the ", group2, "group",description,".")))
}
```
...
```{r}
Hunger54.ovrlavg <- mean(data$Q54b, na.rm = TRUE) * 100
#Poverty28.gendert <- t.test(Q28pd ~ Q9b, data=data) 

# % of people worried about having enough food
hunger.ttest <- t.test(Q54b ~ designtype, data = joineddata)

```
`r round(hunger.ttest$estimate[1]*100,2)`% of participants in the `r group1` group worried about having enough food,whereas `r round(hunger.ttest$estimate[2]*100,2)`% of participants in the `r group2` group worried about having enough food. `r text(hunger.ttest, "worried about not having enough food.")`

## Healthy Lives
### Diseases
```{r}
# overall data
data1HealthyF170.ovrlavg <- mean(data1$Q170b, na.rm = TRUE) * 100
data2HealthyF170.ovrlavg <- mean(data2$Q170b, na.rm = TRUE) * 100
DifHealthyF170.num <- data1HealthyF170.ovrlavg - data2HealthyF170.ovrlavg
# HIV
data1Healthy65hiv.num <- sum(data1$HIV == TRUE, na.rm=TRUE)
data2Healthy65hiv.num <- sum(data2$HIV == TRUE, na.rm=TRUE)
DifHealthy65hiv.num <- data1Healthy65hiv.num-data2Healthy65hiv.num
# Tuberculosis
data1Healthy65tub.num <- sum(data1$tuberculosis == TRUE, na.rm=TRUE)
data2Healthy65tub.num <- sum(data2$tuberculosis == TRUE, na.rm=TRUE)
DifHealthy65tub.num <- data1Healthy65tub.num-data2Healthy65tub.num
# Malaria
data1Healthy65mal.num <- sum(data1$malaria == TRUE, na.rm=TRUE)
data2Healthy65mal.num <- sum(data2$malaria == TRUE, na.rm=TRUE)
DifHealthy65mal.num <- data1Healthy65mal.num-data2Healthy65mal.num
# Hepatitis B
data1Healthy65hepb.num <- sum(data1$hepatitisb == TRUE, na.rm=TRUE)
data2Healthy65hepb.num <- sum(data2$hepatitisb == TRUE, na.rm=TRUE)
DifHealthy65hepb.num <- data1Healthy65hepb.num-data2Healthy65hepb.num
```


There was a `r round(DifHealthyF170.num,2)`% decrease of the numbers of participants who had someone in their household been affected with a disease in the last 12 months.The following are the for specific diseases:

i.  HIV - `r DifHealthy65hiv.num` decrease on HIV.

ii. Tuberculosis - `r DifHealthy65tub.num` decrease on tuberculosis.

iii. Malaria - `r DifHealthy65mal.num` decrease on malaria.

iv. Hepatitis B - `r DifHealthy65hepb.num` decrease on Hepatitis B.

## Education
```{r warnings = FALSE, error = TRUE}
# % with some _____ school completed
primary.ttest <- t.test(Q114b_primary~designtype, data=joineddata)
tech.ttest <- t.test(Q114b_technical~designtype, data=joineddata)
tertiary.ttest <- t.test(Q114b_tertiary~designtype, data=joineddata)
none.ttest <- t.test(Q114b_none~designtype, data=joineddata)

joineddata %>%
  group_by(designtype) %>%
  summarize(primary = mean(Q114b_primary, na.rm = TRUE),
            tech = mean(Q114b_technical, na.rm = TRUE),
            tertiary = mean(Q114b_tertiary, na.rm = TRUE),
            none = mean(Q114b_none, na.rm = TRUE)) %>%
  pivot_longer(cols = primary:none, names_to = "education_type", values_to = "participation") %>%
  ggplot(aes(x = education_type, y = participation, fill = designtype)) + 
  geom_col(position = "dodge") +
  ylim(0,1) +
  scale_fill_manual(values=c("#99CCCC","#BBDDDD"),
                         name="Design Type",
                    labels = c(eval(group1), eval(group2)))+
  labs(
    title = "Change in Education Access",
    x = "Education Level",
    y = "Proportion")
...
`r none.ttest$estimate[1]*100`% of participants in the `r group1` received no education in the last 12 months,whereas `r none.ttest$estimate[2]*100`% of participants in the `r group2` received no education. `r text(none.ttest, "received no education.")`

`r primary.ttest$estimate[1]*100`% of participants in the `r group1` received a primary education in the last 12 months,whereas `r primary.ttest$estimate[2]*100`% of participants in the `r group2` received a primary education. `r text(primary.ttest, "received a primary education.")`

`r tech.ttest$estimate[1]*100`% of participants in the `r group1` received a technical education in the last 12 months,whereas `r tech.ttest$estimate[2]*100`% of participants in the `r group2` received a technical education. `r text(tech.ttest, "received a technical education.")`

`r tertiary.ttest$estimate[1]*100`% of participants in the `r group1` received a tertiary education in the last 12 months,whereas `r tertiary.ttest$estimate[2]*100`% of participants in the `r group2` received a tertiary education. `r text(tertiary.ttest, "received a tertiary education.")`
...
plot <- joineddata %>%
  filter(joineddata$Q88 %in% levels) %>%
  ggplot(aes(x = factor(joineddata$Q88,levels=levels,ordered=TRUE), fill=designtype))+
  geom_bar(show.legend = TRUE, position="dodge") +
  scale_x_discrete(labels= c("Less safe from physical, sexual, or psychological violence than other households" = "Less","About the same level of safety from physical, sexual, or psychological violence as other households" = "Same","More safe safe from physical, sexual, or psychological violence than other households" = "More")) +
    scale_fill_manual(values=c("#99CCCC","#BBDDDD"),
                         name="Data Design Type",
                         labels=c(eval(group1), eval(group2)))+
  labs(
    title = expression(atop("Comparison on", "Women's Health")),
    x = "",
...
```{r}
water <- joineddata %>%
  mutate(never = ifelse(Q191 == "Never (0)",1,0))%>%
  mutate(rarely = ifelse(Q191 == "Rarely (1-2 times in the prior 4 weeks)",1,0)) %>%
  mutate(often = ifelse(Q191 == "Often (11-20 times in the prior 4 weeks)",1,0)) %>%
  mutate(always = ifelse(Q191 == "Always (above 20 times in prior 4 weeks)",1,0)) %>%
  select(designtype, never:always)
  
never.ttest <- t.test(never ~ designtype,data=water)
always.ttest <- t.test(always ~ designtype, data=water)

water %>%
  pivot_longer(cols = never:always, names_to = "clean_water_access") %>%
  group_by(designtype, clean_water_access) %>%
  summarize(avg = mean(value,na.rm = TRUE)) %>%
  ggplot(aes(x = clean_water_access, y = avg, fill = designtype)) + 
  geom_col(position = "dodge") +
  ylim(0,1) +
  scale_fill_manual(values=c("#99CCCC","#BBDDDD"),
                         name="Clean Water Access",
                    labels = c(eval(group1), eval(group2)))+
  scale_x_discrete(labels = c("Never", "Sometimes", "Often", "Always"))+
  labs(
    title = "Change in Clean Water Access",
    x = "",
    y = "Proportion")
```

`r never.ttest$estimate[1]*100`% of participants in the `r group1` reported that in the last 4 weeks, neither they nor anyone in their household drank water that they thought was unsafe, whereas `r never.ttest$estimate[2]*100`% of participants in the `r group2` reported that in the last 4 weeks, neither they nor anyone in their household drank water that they thought was unsafe. `r text(never.ttest, "reported that they never drank water that they thought was unsafe.")`

`r always.ttest$estimate[1]*100`% of participants in the `r group1` reported that in the last 4 weeks, they or someone in their household always drank water that they thought was unsafe, whereas `r always.ttest$estimate[2]*100`% of participants in the `r group2` reported that in the last 4 weeks, they or someone in their household always drank water that they thought was unsafe. `r text(always.ttest, "reported that they always drank water that they thought was unsafe.")`
...
plot <- joineddata %>%
  filter(joineddata$Q56 %in% levels) %>%
  filter(!is.na(joineddata$Q56)) %>%
  ggplot(aes(x = factor(joineddata$Q56,levels=levels,ordered=TRUE), fill=joineddata$designtype))+
  geom_bar(show.legend = TRUE, position="dodge") +
  scale_x_discrete(labels= c("Worse access to electricity than most households" = "Worse","About the same access to electricity as most households" = "Same","Better access to electricity than most households" = "Better"))+
  scale_fill_manual(values=c("#99CCCC","#BBDDDD"),
                         name="Data Design Type",
...
plot3 <- joineddata %>%
  filter(Q139c <= 30) %>%
  filter(!is.na(Q9b)) %>%
  ggplot( aes(x=Q139c, fill=designtype))+
  geom_boxplot() +
  ggtitle("Hourly Wage by Gender") + 
  xlab("Hourly Wage") + ylab("Density") +
scale_fill_manual(values=c("#99CCCC","#BBDDDD"),
                         name="Data Design Type",
                         labels=c(eval(group1), eval(group2))) +
  facet_grid(facets = ~Q9b,
             labeller = labeller(Q9b = gender.labs)) +
...
```{r}
#Homelessness extremes - house with roof and walls vs no place to call home
plot <- ggplot(joineddata, aes(x=Q46, fill = designtype))+
  geom_bar(show.legend = TRUE, position = "dodge") +
  ggtitle("Housing Situation") + 
 # scale_fill_manual(values=c("#99CCCC","#99CCCC","#99CCCC","#99CCCC","#99CCCC")) +
  scale_x_discrete(labels=c("I don't have a place to call home.","I have a home that is missing one or more walls or roof and has a dirt floor."=expression(atop("I have a home that is missing one or","more walls or roof and has a dirt floor")),"I have a home that includes temporary walls or roof and has a dirt floor."=expression(atop("I have a home that includes temporary","walls or roof and has a dirt floor")),"I have a home with solid walls and floor."=expression(atop("I have a home with","solid walls and floor"))))+
